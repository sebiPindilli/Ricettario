...
export async function parseRecipeFromLink({ url, text }) {
  ...
  if (fetchedHtml) {
    ...
    // che raggiunge la pagina in modo indiretto.
    const truncatedHtml = fetchedHtml.substring(0, 120000);
    payload = {
      contents: [{
        parts: [
          { text: PROMPT_BASE },
          { text: `Ecco il contenuto HTML della pagina da analizzare per estrarre la ricetta:\n\n${truncatedHtml}` },
        ],
      }],
      generationConfig: { responseMimeType: "application/json", temperature: 0 },
    };
